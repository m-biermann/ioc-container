...
BOOST_AUTO_TEST_SUITE(Dependency)
	...
	BOOST_AUTO_TEST_CASE(Simple)
	{
		...
		uut->RegisterSingleton(std::function([](Container::Dependency<A> a, unsigned value){return std::make_shared<B>(a, value);}));
	...
	BOOST_AUTO_TEST_CASE(Injection)
	{
		...
		uut->RegisterSingleton(std::function([](Container::Injection<IC> c) {return std::make_shared<D>(std::make_shared<B>(std::make_shared<A>(3), 4), c, 8);}));
	...
	BOOST_AUTO_TEST_CASE(InjectionWithId)
	{
		...
		uut->RegisterTransient(std::function([](Container::InjectionRuntimeResolved<IC> c) {
			return std::make_shared<D>(std::make_shared<B>(std::make_shared<A>(3), 4), c, 8);
		}));
